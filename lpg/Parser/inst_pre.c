...

{
  ...
  if ( gorig_initial_facts ) {
    ...
    */
	...
	NB.
	    Rialloco se gnum_fullnum_initial è pari a max_num_value-1 perchè gli effetti numerici (in gcomp_var_effects)
	    assumeranno indici da 1 in poi (manca lo zero, perchè poi saranno usati indici negativi, a partire da -1,
	    per distinguerli dai fatti booleani). Quindi l'ultimo effetto inserito è quello in gnum_fullnum_initial
	...
	/*
	  ...
	  (potrei anche farlo dopo fuori dal ciclo... ma così tutte le realloc sono insieme)
	*/
...

{
  ...
  if (GpG.derived_predicates) {
	...
	expand_quantifiers_in_wff(&(gderivedpred[i]->preconds), -1, -1);
...

{
  ...
  sprintf( gpredicates[gnum_predicates], "%s%s", STR_NOT_MINUS, gpredicates[p] );
