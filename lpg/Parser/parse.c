...
{
  PlOperator *tmpop, *auxop, *prev, *to_del;
  PlNode *pln;
  ...
  if (GpG.timed_facts_present) {
    for (tmpop = plop; tmpop; tmpop = tmpop -> next) {
      if (tmpop -> ops_type == TIMED_FACT_ACT) {
	auxop = tmpop -> next;
	prev = tmpop;
	while (auxop) {
	  if ((auxop -> ops_type == TIMED_FACT_ACT) &&  ((tmpop -> duration -> value) == (auxop -> duration -> value)) ) {
	  
	    if (DEBUG3) {
	      printf("\nTempo : %f", tmpop -> duration -> value);
	      printf("\nUnisco i timed facts %s e %s", tmpop -> name, auxop -> name);
	    }
	    
	    to_del = auxop;
	    if (tmpop -> effects -> connective != AND) {
	      pln = new_PlNode(AND);
	      pln -> sons = tmpop -> effects;
	      tmpop -> effects = pln;
	    }
	    for (pln = tmpop -> effects -> sons; pln -> next; pln = pln -> next);
	    pln -> next = cp_PlNode(auxop -> effects);
	    prev -> next = auxop -> next;
	    auxop = auxop -> next;
	    to_del -> next = NULL;
	    free_PlOperator(to_del);
	    gnum_tmd_init_fcts--;
	  }
	  else {
	    prev = prev -> next;
...
/* Makes TRU the nodes to be removed */
  ...
  if ((*pln)->next)
	...
	case AT_START_CONN: nodetoremove->sons->is_start_end_ovr = IS_AT_START;
	  break;
	case AT_END_CONN: nodetoremove->sons->is_start_end_ovr = IS_AT_END;
	  break;
	case OVER_ALL_CONN: nodetoremove->sons->is_start_end_ovr = IS_OVERALL;
	default:
	  break;
	}
