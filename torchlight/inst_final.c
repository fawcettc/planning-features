...

{
  ...
  for ( i = 0; i < gnum_ft_conn; i++ ) {
    ...
    gft_conn[i].notFD = FALSE;
  }

  for ( i = 0; i < gnum_op_conn; i++ ) {
    gop_conn[i].num_E = 0;
    gop_conn[i].num_eff = 0;
    gop_conn[i].num_pre = 0;
  }
  ...
  for ( i = 0; i < gnum_op_conn; i++ ) {
    gop_conn[i].is_recoverer = FALSE;
  }



  /* in this version, we want ONLY STRIPS! Has been verified earlier,
     just double-check here.
   */
  for ( i = 0; i < gnum_op_conn; i++ ) {
    if ( gop_conn[i].num_E > 1 ) {
      printf("\n\nOp with %d > 1 effect? ", gop_conn[i].num_E);
      print_op_name(i);
      printf("\n\n");
      exit( 1 );
    }
