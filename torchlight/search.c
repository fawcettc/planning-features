...

{
  ...
  if ( !gdoing_EHCanalyze ) {
    ...
    if ( gcmd_line.display_info == 2 ) {
      printf("\n\nLocal monotone EHC at goal distance: %4d and depth ", h);
    }
    return search_for_better_state( &S, h, &S_, &h_ );
  }
...

{
  ...
  while ( TRUE ) {  
    ...
    if ( lehc_current_start->depth > depth ) {
      ...
      if ( gEHC_max_search_depth == -1 ||
	   depth > gEHC_max_search_depth ) {
	gEHC_max_search_depth = depth;
      }
      if ( !gdoing_EHCanalyze ) {
	printf("[%d]", depth);
	fflush( stdout );
...
/********************************************************
 * state sampling routine for TorchLight
 ********************************************************/
...

{
  ...
  /* make space
   */
  gsample_states = ( State * ) calloc( gcmd_line.num_samples, sizeof( State ) );
  for ( i = 0; i < gcmd_line.num_samples; i++ ) {
    make_state( &(gsample_states[i]), gnum_ft_conn ); 
  }
  make_state( &s, gnum_ft_conn ); 
  make_state( &s_, gnum_ft_conn ); 
  ...
  /* get number of steps
   */
  ini_h = get_1P( &ginitial_state, &ggoal_state );
  if ( ini_h == INFINITY ) {
    gcmd_line.num_samples = 0;
    return;
  }
  ...
  i = 0;
  while ( i < gcmd_line.num_samples ) {
    /* sample a state
     */
    number_steps = random() % (gcmd_line.depth_samples * ini_h);
	...
	break;
      ...
      next_step = random() % gnum_A;
      if ( 0 ) {
	printf("\nsample op: %d of %d, id %d ", next_step, gnum_A, gA[next_step]);
	print_op_name( gA[next_step] );
	fflush(stdout);
      }

      result_to_dest( &s_, &s, gA[next_step] );
      source_to_dest( &s, &s_);
    }

    /* if s is actually a goal state, reject it!
     */
    for ( j = 0; j < ggoal_state.num_F; j++ ) {
      for ( k = 0; k < s.num_F; k++ ) {
	if ( s.F[k] == ggoal_state.F[j] ) {
	  break;
	}
      }
      if ( k == s.num_F ) {
    ...
    if ( j == ggoal_state.num_F ) {
      continue;
    }

    source_to_dest( &(gsample_states[i]), &s);

    if ( gcmd_line.display_info > 1 ) {
      printf("\nSample state %d: ", i);
      print_state(gsample_states[i]);
    }
